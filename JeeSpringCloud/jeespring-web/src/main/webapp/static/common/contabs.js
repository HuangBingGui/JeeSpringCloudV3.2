...
function closeSelectTabsDo(){
    $('.page-tabs-content').children("[data-id]").not(":first").filter(".active").each(function () {
        $('.J_iframe[data-id="' + $(this).data('id') + '"]').remove();
        $(this).remove();
    });
    $('.page-tabs-content').css("margin-left", "0");
    $('.page-tabs-content').children("[data-id]").filter(":last").addClass("active").each(function () {
        $('.J_iframe[data-id="' + $(this).data('id') + '"]').show();
    });
    scrollToTab($('.J_menuTab.active'));
}
...
//打开选项卡菜单
function openTabView(title,url,width,height){
    openTab(url,title,true)
}
function openTabEdit(title,url,width,height){
    openTab(url,title,true)
}
